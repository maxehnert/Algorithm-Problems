...
A good general strategy for thinking about an algorithm is to try writing out a sample input and performing the operation by hand. If youre stuck, try that!
...
To start, lets just write a function that chooses the 0th element for our sorted array.
...
We need to make sure we dont try to put that 1 in merged_array again. We should mark it as "already merged" somehow. For now, we can just cross it out:

  my_array     = [3,  4,  6, 10, 11, 15]
alices_array = [x,  5,  8, 12, 14, 19]
merged_array = [1,  x,  x,  x,  x,  x]
Or we could even imagine its removed from the array:
...
Well start at the beginnings of our input arrays, since the smallest elements will be there.
...
Same for these two lines:
  ...
  merged_array[current_index_merged] = alices_array[current_index_alices]
current_index_alices += 1
Thats not DRY ↴ . Maybe we can avoid repeating ourselves by bringing our code back down to just 2 cases.
...
You might try to simply squish the first and last cases together:
  ...
  if is_alices_array_exhausted or \
    ...
    merged_array[current_index_merged] = my_array[current_index_mine]
    current_index_mine += 1
But what happens when my_array is exhausted? Well get and IndexError when we try to access my_array[current_index_mine]!
...
Also, in Python 2.6+, theres a built-in function for merging sorted arrays into one sorted array: heapq.merge().
